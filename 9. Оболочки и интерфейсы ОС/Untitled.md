...
Это один из первых методов взаимодействия с операционной системой, который давно устарел (что за бред? Откуда вы это всё копипастите?).
...
«[[Графический интерфейс]]ы бывают разными: существуют [[ОС]] без графического [[Графический интерфейс]]а, есть программы без графического интерфейса, которые запускаются чисто в  текстовом виде, существуют программы без [[Графический интерфейс]]а, которые выполняются сами по себе и не подразумевают взаимодействия с пользователем (например, .bat файл).»

	Командная строка (в дополнение файловый менеджер)
«Можно предположить, что первым интерфейсом являлась перфокарта, при помощи которой пользователь мог хоть как-то общаться с компьютером, но, скажем так, здесь есть пограничная история: это просто устройство ввода/вывода, но тогда не было каких-либо других способов коммуникации с компьютером, поэтому, в принципе, это можно назвать интерфейсом.
...
[[GUI]] (Graphical User Interface) или ГИП (графический [[Графический интерфейс]] пользователя) — это одна из разновидностей пользовательских [[Графический интерфейс|графических интерфейсов]], элементы которого выполнены в виде графических изображений.
